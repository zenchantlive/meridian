"""
MERIDIAN Brain - REMEMBER Operation Tests
...
try:
    ...
    from brain.scripts.auto_linker import AutoLinker, LinkGraph
    ...
    from auto_linker import AutoLinker, LinkGraph
...
class TestRememberSideEffects(unittest.TestCase):
    ...
    def test_link_graph_index_updated(self):
        """Link graph index should be updated with new links."""
        # First create a chunk to link against
        self.remember.remember(
            content="First chunk in conversation",
            conversation_id="link-graph-test",
            tags=["link-test"]
        )
        ...
        # Verify link graph file exists and is valid  
        link_graph_path = Path(self.linker.link_graph.index_path)
        self.assertTrue(link_graph_path.exists(),
                       f"Link graph index should exist at {link_graph_path}")
        
        data = json.loads(link_graph_path.read_text(encoding="utf-8"))
        self.assertIn("forward", data)
        self.assertIn("reverse", data)
